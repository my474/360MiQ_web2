...
assert.strictEqual((pineDocsList.innerHTML.match(/data-sce-pine-doc-insert-text="and"/g) || []).length, 1);
assert.strictEqual((pineDocsList.innerHTML.match(/title="Insert and into the editor"/g) || []).length, 1);
...
chart.renderPineDocumentation(pineDocsSearch.value, pineDocsFilter.value);
const plotDocumentationMatch = pineDocsList.innerHTML.match(/data-sce-pine-doc-insert="(\d+)"[^>]*title="Insert plot/);
assert.ok(plotDocumentationMatch);
