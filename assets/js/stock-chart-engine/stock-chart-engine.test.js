...
const engineCss = fs.readFileSync(path.join(__dirname, 'stock-chart-engine.css'), 'utf8');
const lightThemeCss = engineCss.match(/\.sce-root\s*\{([\s\S]*?)\n\}/);
const darkThemeCss = engineCss.match(/\.sce-root\[data-sce-theme="dark"\]\s*\{([\s\S]*?)\n\}/);
assert.ok(lightThemeCss, 'light chart theme CSS must exist');
assert.ok(darkThemeCss, 'dark chart theme CSS must exist');
assert.match(lightThemeCss[1], /--sce-latest-bg:\s*#facc15;/);
assert.match(lightThemeCss[1], /--sce-latest-border:\s*#a16207;/);
assert.match(lightThemeCss[1], /--sce-latest-hover-bg:\s*#eab308;/);
assert.match(lightThemeCss[1], /--sce-latest-text:\s*#422006;/);
assert.match(darkThemeCss[1], /--sce-latest-bg:\s*#facc15;/);
assert.match(darkThemeCss[1], /--sce-latest-border:\s*#fde68a;/);
assert.match(darkThemeCss[1], /--sce-latest-hover-bg:\s*#fde047;/);
assert.match(darkThemeCss[1], /--sce-latest-text:\s*#1c1917;/);
assert.match(engineCss, /\.sce-latest-marker\s*\{[\s\S]*?color:\s*var\(--sce-latest-text\);/);
assert.match(engineCss, /\.sce-latest-marker:hover,[\s\S]*?background:\s*var\(--sce-latest-hover-bg\);/);
...
function assertCrosshairAxisMarkers(chart, expectedThemeName) {
  ...
  const seriesMarkerTops = chart.canvas.commands.filter((command) => (
    command.type === 'moveTo' && command.x === rect.scaleX + 9
  ));
  const seriesMarkerBottoms = chart.canvas.commands.filter((command) => (
    command.type === 'lineTo' && command.x === rect.scaleX + 9
  ));
  ...
  assert.strictEqual(seriesMarkerBottoms.length, persistentBackgrounds.length, `${expectedThemeName} series triangles should begin at the marker bottom`);
...
assert.strictEqual(chart.hitTestStrategyExecution({ x: executionZone.x + 2, y: executionZone.y + 2 }).execution.orderId, executionZone.execution.orderId);
chart.showStrategyTooltip(executionZone.execution, { x: executionZone.x, y: executionZone.y });
assert.strictEqual(chart.strategyTooltip.hasAttribute('hidden'), false);
chart.hideStrategyTooltip();
assert.strictEqual(chart.strategyTooltip.hasAttribute('hidden'), true);
chart.setTheme('dark');
...
assert.ok(pineDocsList.innerHTML.includes('class="sce-pine-doc-list-item"'));
assert.strictEqual((pineDocsList.innerHTML.match(/data-sce-pine-doc-insert-text="and"/g) || []).length, 0);
assert.strictEqual((pineDocsList.innerHTML.match(/title="Insert and into the editor"/g) || []).length, 0);
assert.ok(pineDocsDetail.innerHTML.includes('data-sce-pine-doc-insert-text="and"'));
assert.ok(pineDocsDetail.innerHTML.includes('title="Insert and into the editor"'));
...
const pineCompletions = new FakeElement('div');
pineCompletions.setAttribute('data-sce-pine-completions', '');
chart.settingsPopup.appendChild(pineSignatureField);
chart.settingsPopup.appendChild(pineSignatureHelp);
chart.settingsPopup.appendChild(pineCompletions);
pineSignatureField.value = 'in';
pineSignatureField.selectionStart = pineSignatureField.value.length;
pineSignatureField.selectionEnd = pineSignatureField.value.length;
chart.showPineCompletions(false);
assert.strictEqual(typeof pineCompletions.onclick, 'function');
pineSignatureField.selectionStart = 0;
pineSignatureField.selectionEnd = 0;
const indicatorCompletion = new FakeElement('strong');
indicatorCompletion.setAttribute('data-sce-pine-completion', 'indicator');
pineCompletions.onclick({ target: indicatorCompletion, preventDefault() {}, stopPropagation() {} });
assert.strictEqual(pineSignatureField.value, 'indicator');
pineSignatureField.value = 'input.';
pineSignatureField.selectionStart = pineSignatureField.value.length;
pineSignatureField.selectionEnd = pineSignatureField.value.length;
chart.showPineCompletions(false);
pineSignatureField.selectionStart = 0;
pineSignatureField.selectionEnd = 0;
const inputCompletion = new FakeElement('strong');
inputCompletion.setAttribute('data-sce-pine-completion', 'input.int');
pineCompletions.onclick({ target: inputCompletion, preventDefault() {}, stopPropagation() {} });
assert.strictEqual(pineSignatureField.value, 'input.int');
pineSignatureField.value = 'indicat';
pineSignatureField.selectionStart = pineSignatureField.value.length;
pineSignatureField.selectionEnd = pineSignatureField.value.length;
const pineCompletionName = new FakeElement('strong');
pineCompletionName.setAttribute('data-sce-pine-completion', 'indicator');
chart.settingsPopup.onclick({ target: pineCompletionName, preventDefault() {}, stopPropagation() {} });
assert.strictEqual(pineSignatureField.value, 'indicator');
pineSignatureField.value = 'plot(ta.sma(close, 20), ';
pineSignatureField.selectionStart = pineSignatureField.value.length;
pineSignatureField.selectionEnd = pineSignatureField.value.length;
...
pineDocsSearch.value = 'plot';
pineDocsFilter.value = 'function';
chart.renderPineDocumentation(pineDocsSearch.value, pineDocsFilter.value);
const plotRowMatch = pineDocsList.innerHTML.match(/data-sce-pine-doc-index="(\d+)"[^>]*><strong>plot<\/strong>/);
assert.ok(plotRowMatch);
chart.showPineDocumentationEntry(plotRowMatch[1]);
const plotDetailMatch = pineDocsDetail.innerHTML.match(/data-sce-pine-doc-insert="(\d+)"[^>]*title="Insert plot/);
assert.ok(plotDetailMatch);
pineSignatureField.value = '';
pineSignatureField.selectionStart = 0;
pineSignatureField.selectionEnd = 0;
assert.strictEqual(chart.insertPineDocumentation(plotDetailMatch[1]), true);
...
pineDocsSearch.value = 'ta.supertrend';
pineDocsFilter.value = 'function';
chart.renderPineDocumentation(pineDocsSearch.value, pineDocsFilter.value);
...
pineDocsSearch.value = 'true / false / na';
pineDocsFilter.value = 'constant';
chart.renderPineDocumentation(pineDocsSearch.value, pineDocsFilter.value);
chart.showPineDocumentationEntry(StockChartEngine.pineScriptDocumentation.findIndex((item) => item.name === 'true / false / na'));
assert.ok(pineDocsDetail.innerHTML.includes('true represents a true condition'));
...
const groupedNameHtml = chart.pineDocumentationNameHtml({ name: 'true / false / na', category: 'constant' }, 0, false);
assert.ok(groupedNameHtml.includes('sce-pine-doc-name-group'));
assert.ok(groupedNameHtml.includes('true') && groupedNameHtml.includes('false') && groupedNameHtml.includes('na'));
...
pineSignatureField.selectionEnd = pineSignatureField.value.length;
chart.showPineCompletions(false);
assert.ok(pineCompletions.innerHTML.includes('ta.wma'));
...
chart.setPineWindowAction('minimize');
assert.strictEqual(chart.pineWindowState.minimized, true);
assert.strictEqual(chart.pineWindowState.maximized, false);
assert.ok(chart.settingsPopup.className.split(/\s+/).includes('is-minimized'));
assert.ok(!chart.settingsPopup.className.split(/\s+/).includes('is-maximized'));
chart.setPineWindowAction('minimize');
assert.strictEqual(chart.pineWindowState.minimized, false);
assert.strictEqual(chart.pineWindowState.maximized, false);
