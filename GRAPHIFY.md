...
Keep `blog/` in `.graphifyignore` before rebuilding Graphify. If the WordPress tree needs separate analysis later, run that as a deliberate one-off instead of folding it into the normal project graph.

## Update Workflow

When Graphify needs to be updated, read this file first and confirm `.graphifyignore` still excludes `blog/`. Do not start by scanning the whole project blindly; use this note as the project-specific Graphify guide so vendored WordPress files stay out of the normal graph.
