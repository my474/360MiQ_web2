...
3. Keep PHP account sessions in a private directory outside the document root. When the production `ACCOUNT_DB_INCLUDE` lives in its default private `php_script` directory, the application automatically creates and uses `/home2/aamiqcom/php_script/account_sessions` with owner-only permissions. Other deployments must set `MIQ_ACCOUNT_SESSION_SAVE_PATH` to an absolute path whose parent already exists outside the public web root. The application migrates the current session file on first use and enables PHP garbage collection for the configured 14-day lifetime, preventing cPanel's shared-session cleanup from logging out idle users. Anonymous account pages do not need a PHP session: CSRF tokens, flash messages, and the short-lived Android Google challenge use server-signed, short-lived cookies instead, so crawlers that do not retain cookies cannot create one `sess_*` file per request. Set `MIQ_ACCOUNT_STATE_SECRET` to a stable random value of at least 32 characters; if it is omitted, the application creates a 32-byte owner-only `.miq_account_state_secret` beside the private session directory. Keep that secret stable across deployments; rotating it invalidates pending anonymous forms and Android sign-in challenges.
