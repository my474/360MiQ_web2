...
2. Configure a dedicated account database include with `ACCOUNT_DB_INCLUDE` (the repository template is `mysql_vars_account.php`; deploy it outside the web root, such as `/home2/aamiqcom/php_script/mysql_vars_account.php`), or provide `ACCOUNT_DB_HOST`, `ACCOUNT_DB_NAME`, `ACCOUNT_DB_USER`, `ACCOUNT_DB_PASSWORD`, and optional `ACCOUNT_DB_PORT`.
