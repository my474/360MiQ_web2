...
/**
 * Return the root-relative path where the main application is deployed.
 *
 * MIQ_SITE_URL wins when it contains a path. Otherwise infer the path from
 * the executing script so a copied staging install under /full keeps all
 * account API and shared-asset requests inside /full. WordPress runs one
 * directory deeper under /blog, so that suffix is removed for shared files.
 */
function miq_account_deployment_path($context = 'main')
{
    $configured_path = parse_url(miq_account_config()['base_url'], PHP_URL_PATH);
    if (is_string($configured_path) && trim($configured_path, '/') !== '') {
        return '/' . trim(preg_replace('#/+#', '/', $configured_path), '/');
    }

    $script_name = str_replace('\\', '/', (string) ($_SERVER['SCRIPT_NAME'] ?? ''));
    $request_directory = str_replace('\\', '/', dirname($script_name !== '' ? $script_name : '/index.php'));
    $request_directory = preg_replace('#/+#', '/', $request_directory);

    if ($context === 'blog') {
        if (preg_match('#^(.*?)/blog(?:/.*)?$#', $request_directory, $matches)) {
            $request_directory = $matches[1];
        } else {
            $request_path = parse_url((string) ($_SERVER['REQUEST_URI'] ?? ''), PHP_URL_PATH);
            if (is_string($request_path) && preg_match('#^(.*?)/blog(?:/|$)#', $request_path, $matches)) {
                $request_directory = $matches[1];
            }
        }
    }

    $request_directory = '/' . trim((string) $request_directory, '/');
    return $request_directory === '/' || $request_directory === '/.' ? '' : rtrim($request_directory, '/');
}

function miq_account_public_path($path, $context = 'main')
{
    return miq_account_deployment_path($context) . '/' . ltrim((string) $path, '/');
}
