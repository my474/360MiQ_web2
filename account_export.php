...
$notification_deliveries = miq_account_table('notification_deliveries');
...
$payload = array(
    ...
    'notification_preferences' => miq_account_notification_preferences($user_id),
    'notification_devices' => miq_account_fetch_all(miq_account_query("SELECT id, channel, label, app_version, enabled, last_seen_at, created_at, updated_at FROM {$notification_devices} WHERE user_id = ? ORDER BY updated_at DESC", 'i', array($user_id))),
    'notification_delivery_history' => miq_account_fetch_all(miq_account_query("SELECT notification_id, device_id, status, attempt_count, error_code, http_status, attempted_at, delivered_at, created_at, updated_at FROM {$notification_deliveries} WHERE user_id = ? ORDER BY created_at DESC, id DESC", 'i', array($user_id))),
